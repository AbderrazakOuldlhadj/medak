import os
import sys
import re
import time
import requests
from PIL import Image
from playwright.sync_api import sync_playwright

sys.path.append(os.path.join(r'c:\Users\msipc\Desktop\products\parfums', 'scripts', 'processing'))
from process_image_assets import format_image_to_square_webp

sys.stdout.reconfigure(encoding='utf-8')

def fetch_dior_sauvage_all():
    target_dir = r'c:\Users\msipc\Desktop\products\parfums\assets\product_images\DIOR SAUVAGE EDT 100ML'
    os.makedirs(target_dir, exist_ok=True)

    url = 'https://www.dior.com/en_int/beauty/products/sauvage-eau-de-toilette-Y0685240.html'

    print("Launching Playwright to fetch all candidate images for Dior Sauvage EDT...")
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, args=['--disable-blink-features=AutomationControlled'])
        context = browser.new_context(user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36')
        page = context.new_page()
        page.goto(url, wait_until='domcontentloaded', timeout=30000)
        time.sleep(4)

        img_urls = page.eval_on_selector_all(
            'img',
            'elements => elements.map(e => e.src || e.getAttribute("data-src") || "").filter(s => s)'
        )

        all_candidate_urls = []
        for u in img_urls:
            u_lower = u.lower()
            if any(x in u_lower for x in ['logo', 'icon', 'bag', 'star', 'banner', 'avatar', 'footer', 'header', 'social', 'flag']):
                continue
            high_res = re.sub(r'sw=\d+', 'sw=1500', u)
            high_res = re.sub(r'sh=\d+', 'sh=1500', high_res)
            if high_res not in all_candidate_urls:
                all_candidate_urls.append(high_res)

        print(f"Found {len(all_candidate_urls)} candidate image URLs:")
        for cu in all_candidate_urls:
            print("  ->", cu)

        session = requests.Session()
        session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})

        saved = 0
        for idx, u in enumerate(all_candidate_urls, 1):
            try:
                r = session.get(u, timeout=12)
                if r.status_code == 200 and len(r.content) > 3000:
                    ext = 'png' if u.lower().endswith('.png') else 'jpg'
                    raw_name = f'raw_{idx}.{ext}'
                    raw_path = os.path.join(target_dir, raw_name)
                    with open(raw_path, 'wb') as f:
                        f.write(r.content)
                    
                    img = Image.open(raw_path)
                    webp_name = 'bottle.webp' if idx == 1 else f'image_{idx}.webp'
                    webp_path = os.path.join(target_dir, webp_name)
                    
                    success = format_image_to_square_webp(img, webp_path, remove_bg=True)
                    if success:
                        print(f'  [✓ Downloaded & Kept Image {idx}] {raw_name} -> {webp_name} ({img.width}x{img.height}px, {len(r.content)//1024} KB)')
                        saved += 1
            except Exception as e:
                print(f'  Error downloading {u}: {e}')

        browser.close()

    print(f"Done! Preserved all {saved} downloaded images in DIOR SAUVAGE EDT 100ML/")

if __name__ == '__main__':
    fetch_dior_sauvage_all()
