...
def fetch_dior_elixir():
    target_dir = r'c:\Users\msipc\Desktop\products\parfums\assets\product_images\DIOR SAUVAGE ELEXIR EDP 100ML'
    ...
    url = 'https://www.dior.com/en_int/beauty/products/sauvage-elixir-Y0996460.html'

    print("Launching Playwright for Dior Sauvage Elixir Y0996460...")
    with sync_playwright() as p:
        ...
        for u in img_urls:
            u_lower = u.lower()
            if any(x in u_lower for x in ['logo', 'icon', 'bag', 'star', 'banner', 'avatar', 'footer', 'header', 'social', 'flag', 'navigation']):
                continue
            if 'Y0996460' in u or 'elixir' in u_lower or 'demandware.static/-/sites-master_dior' in u_lower:
                if u not in candidate_urls:
                    candidate_urls.append(u)

        print(f"Extracted {len(candidate_urls)} candidate Dior Sauvage Elixir product URLs:")
    ...
    print(f"Done! Successfully saved {saved} authentic Dior Sauvage Elixir photos into DIOR SAUVAGE ELEXIR EDP 100ML/")
