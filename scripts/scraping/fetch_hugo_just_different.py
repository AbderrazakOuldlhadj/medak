...
def fetch_hugo_just_different():
    target_dir = r"c:\Users\msipc\Desktop\products\parfums\assets\product_images\HUGO BOSS JUST DIFFRENT EDT 75ML"
    ...
    url = 'https://www.flaconi.fr/parfum/hugo-boss/just-different/hugo-boss-just-different-eau-de-toilette.html?variant=80075625-75'

    print(f"Launching Playwright for Flaconi URL: {url}")
    candidate_urls = []
    ...
    with sync_playwright() as p:
        ...
        for selector in ['#uc-btn-accept-banner', 'button:has-text("Tout accepter")', 'button:has-text("Accepter")']:
            try:
        ...
        for u in img_urls:
            for sub_u in u.split(','):
                ...
                if any(x in c_lower for x in ['logo', 'icon', 'badge', 'flag', 'star', 'banner', 'avatar', 'footer', 'header', 'payment', 'facebook', 'instagram', 'youtube']):
                    continue
                if 'flaconi' in c_lower or 'just' in c_lower or 'hugo' in c_lower or 'product' in c_lower:
                    high_res = re.sub(r'width=\d+', 'width=1500', raw_u)
                    high_res = re.sub(r'height=\d+', 'height=1500', high_res)
                    if high_res not in candidate_urls:
                        candidate_urls.append(high_res)

        browser.close()

    # Search Odorem DZ for Hugo Boss Just Different
    session = requests.Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'})
    try:
        od_res = session.get('https://odorem-dz.com/?s=just+different&post_type=product', timeout=12)
        if od_res.status_code == 200:
            soup = BeautifulSoup(od_res.text, 'html.parser')
            for a in soup.find_all('a', href=True):
                if '/produit/' in a['href'] and 'different' in a['href'].lower():
    ...
    print(f"Found {len(candidate_urls)} candidate Hugo Boss Just Different photos:")
    ...
    print(f"Done! Successfully saved {saved} authentic Hugo Boss Just Different EDT 75ml photos into HUGO BOSS JUST DIFFRENT EDT 75ML/")

if __name__ == '__main__':
    fetch_hugo_just_different()
