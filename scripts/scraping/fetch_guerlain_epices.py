...
def fetch_guerlain_epices():
    target_dir = r"c:\Users\msipc\Desktop\products\parfums\assets\product_images\GUERLAIN EPICES EXQUISES EDP 125ML"
    ...
    urls = [
        'https://briki-parfums.com/parfums-homme/guerlain-epices-exquises/',
        'https://odorem-dz.com/produit/guerlain-epices-exquises-125ml-edp/'
    ]
    ...
    for u in urls:
        try:
            ...
            if res.status_code == 200:
                ...
                gallery = soup.find_all(class_=re.compile(r'woocommerce-product-gallery|product-gallery|gallery|product-image|media', re.I))
                for g in gallery:
                    ...
                    for img in g.find_all('img'):
                        val = img.get('data-large_image') or img.get('src') or img.get('data-src') or img.get('srcset')
                        if val:
                            for sub in val.split(','):
                                clean_val = sub.strip().split(' ')[0]
                                if any(ext in clean_val.lower() for ext in ['.jpg', '.png', '.webp']):
                                    img_urls.append(clean_val)
                # Also check general page images
                for img in soup.find_all('img'):
                    src = img.get('src') or img.get('data-src')
                    if src and 'epices' in src.lower():
                        img_urls.append(src)
    ...
    print(f"Done! Successfully saved {saved} authentic Guerlain Épices Exquises EDP 125ml photos into GUERLAIN EPICES EXQUISES EDP 125ML/")

if __name__ == '__main__':
    fetch_guerlain_epices()
