...
def fetch_briki_epices():
    ...
    url = 'https://briki-parfums.com/parfums-homme/guerlain-epices-exquises/'

    candidate_urls = []
    print(f"Launching Playwright for Briki Parfums URL: {url}")
    with sync_playwright() as p:
        ...
        for u in img_urls:
            for sub_u in u.split(','):
                ...
                c_lower = raw_u.lower()
                if any(x in c_lower for x in ['logo', 'icon', 'badge', 'flag', 'star', 'banner', 'avatar', 'footer', 'header', 'payment', 'facebook']):
                    continue
                if any(ext in c_lower for ext in ['.jpg', '.jpeg', '.png', '.webp']):
                    high_res = re.sub(r'-\d+x\d+(\.[a-zA-Z0-9]+)$', r'\1', raw_u)
                    if high_res not in candidate_urls:
                        candidate_urls.append(high_res)

        browser.close()

    # Search Odorem DZ / Notino for Epices Exquises
    session = requests.Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'})
    try:
        od_res = session.get('https://odorem-dz.com/?s=epices+exquises&post_type=product', timeout=12)
        if od_res.status_code == 200:
            soup = BeautifulSoup(od_res.text, 'html.parser')
            for a in soup.find_all('a', href=True):
                if '/produit/' in a['href'] and 'epices' in a['href'].lower():
    ...
    print(f"Found {len(candidate_urls)} candidate Guerlain Épices Exquises photos:")
